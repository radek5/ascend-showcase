...
const ALLOWED_HEADSHOT_MIME_TYPES = new Set(["image/jpeg", "image/png"]);
...
export async function POST(
  ...
  try {
    ...
    const isHeadshot = documentType === "HEADSHOT";
    ...
    const maxSize = isHeadshot ? MAX_HEADSHOT_SIZE : MAX_DOCUMENT_SIZE;
    ...
    const extension =
      filename.split(".").pop()?.toLowerCase() || (isHeadshot ? "jpg" : "pdf");

    const folder = documentType.toLowerCase();

    const storageKey =
      `showcase-applications/${applicationId}/identity/` +
      `${folder}/` +
     ...
     */
    const existing = await prisma.showcaseIdentityDocument.findUnique({
      where: {
        applicationId_type: {
          applicationId,
    ...
    if (existing) {
      document = await prisma.showcaseIdentityDocument.update({
        where: {
          id: existing.id,
        },
        data: {
      ...
      document = await prisma.showcaseIdentityDocument.create({
        data: {
          applicationId,
          type: documentType as "PASSPORT" | "NIN" | "HEADSHOT",
          status: "UPLOADED",
    ...
    console.error("SHOWCASE IDENTITY PRESIGN ERROR", error);

    return NextResponse.json(
      {
        error: "Unable to prepare the identity document upload.",
      },
      { status: 500 },
    );
  }
