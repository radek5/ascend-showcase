...
type VideoType =
  "MATCH_1" | "MATCH_2" | "HIGHLIGHTS" | "ADDITIONAL_MATCH" | "FULL_MATCH";

type InitialVideoType = "MATCH_1" | "MATCH_2" | "HIGHLIGHTS";
...
type VideoRequest = {
  ...
  requestedVideoType: "ADDITIONAL_MATCH" | "FULL_MATCH";
...
export default function VideoPage() {
  ...
  const [uploads, setUploads] = useState<Record<InitialVideoType, UploadState>>(
    {
      MATCH_1: emptyUpload(),
      MATCH_2: emptyUpload(),
  ...
  async function loadApplication() {
    try {
      const response = await fetch(`/api/showcase-applications/${id}/videos`);

      const data = await response.json();

      if (!response.ok) {
        setPageError(data.error || "Application not found.");
        return;
      }
      ...
      setUploads((current) => {
        ...
        for (const type of [
          ...
          const existing = record.videos.find(
            (video) =>
              video.type === type &&
              ["SUBMITTED", "READY"].includes(video.status),
          );
          ...
          if (existing) {
            next[type] = {
              ...
              uploaded: true,
              progress: 100,
              filename: existing.originalFilename || "Video submitted",
            };
          }
        }
      ...
      setPageError("We could not load your video submission.");
    } finally {
      setLoading(false);
    }
  }
  ...
  function updateUpload(type: InitialVideoType, partial: Partial<UploadState>) {
    setUploads((current) => ({
      ...current,
      [type]: {
        ...current[type],
  ...
  async function uploadVideo(type: InitialVideoType) {
    ...
    try {
      ...
      if (!completeResponse.ok) {
        throw new Error(
          complete.error || "Video uploaded but could not be saved.",
        );
      }
  ...
  if (!application) {
    return (
      <main className="min-h-screen bg-[#090909] px-6 py-16 text-white">
        ...
        {pageError ? <p className="mt-4 text-red-300">{pageError}</p> : null}
      </main>
    );
  }
              ...
              >
                ...
                <span
                  className={`text-[10px] font-bold uppercase tracking-[0.08em] ${
                    index === 5 ? "text-white" : "text-white/35"
                  }`}
                >
                  {step}
                </span>
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-6 py-14 lg:grid-cols-[1fr_340px] lg:px-8">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-white/55">
            Submit multiple examples of your football performance so the
            REVELATIONX1 assessment team can evaluate you across different
            situations.
          </p>
          ...
          <div className="mt-8 rounded-2xl border border-white/10 bg-white/[0.025] p-6">
            ...
            <p className="mt-3 text-sm leading-6 text-white/50">
              Three videos are required for the initial assessment. Match
              footage should show enough continuous play for the assessment team
              to understand your positioning, movement, decision-making and
          ...
          {application.videoRequests.length > 0 ? (
            <div className="mt-10 space-y-8">
              {application.videoRequests.map((request, requestIndex) => {
                const submittedVideos = request.videos.filter((video) =>
                  ["SUBMITTED", "PROCESSING", "READY"].includes(video.status),
                );

                const submittedCount = submittedVideos.length;

                const remainingCount = Math.max(
                  request.requestedCount - submittedCount,
                  0,
                );
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-5">
                      <div>
                        <div className="text-xs font-black uppercase tracking-[0.18em] text-[#c7ff2f]">
                          Additional Footage Request {requestIndex + 1}
                        </div>

                        <h2 className="mt-3 text-2xl font-black">
                          REVELATIONX1 has requested {request.requestedCount}{" "}
                          additional{" "}
                          {request.requestedCount === 1 ? "video" : "videos"}
                        </h2>
                      ...
                      <div className="mt-6 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
                        {Array.from({
                          ...
                          <AdditionalVideoUploader
                            ...
                            videoType={request.requestedVideoType}
          ...
          <div className="mt-8 rounded-2xl border border-amber-400/20 bg-amber-400/[0.07] p-6">
            ...
            <p className="mt-3 text-sm leading-6 text-white/60">
              Players who progress to the longlist or final review stage may be
              asked to provide further match footage, additional video evidence
              or full-match footage before a final selection decision is made.
            </p>

            <p className="mt-3 text-sm font-semibold leading-6 text-white">
              Failure to provide requested additional footage within the stated
              deadline may affect progression in the selection process.
            </p>
          </div>
          ...
          {!allRequiredUploaded ? (
            <p className="mt-3 text-right text-xs text-white/35">
              All three assessment videos must be submitted before you can
              continue.
            </p>
          ) : null}
        </div>
        ...
        <aside>
          <div className="sticky top-8 rounded-2xl border border-white/10 bg-white/[0.03] p-6">
            ...
            <div className="mt-6 space-y-4 border-t border-white/10 pt-6 text-sm">
              ...
              <div className="flex justify-between gap-6">
                <span className="text-white/40">Videos required</span>
                <span>3</span>
              </div>
            ...
            <div className="mt-6 rounded-xl border border-[#c7ff2f]/15 bg-[#c7ff2f]/[0.05] p-4 text-sm leading-6 text-white/60">
              Submitted video is used as evidence during the REVELATIONX1 player
              assessment process.
            </div>
          </div>
        </aside>
      ...
      <div className="mt-5 rounded-xl border border-dashed border-white/15 bg-black/20 p-5">
        <div className="flex flex-wrap items-center gap-4">
          <label className="cursor-pointer rounded-full border border-white/15 px-5 py-3 text-xs font-black uppercase tracking-[0.08em] transition hover:border-[#c7ff2f]/50">
            ...
            <input
              ...
              onChange={(event) => onChoose(type, event)}
            />
          </label>

          <div className="min-w-0 flex-1 text-sm text-white/45">
            {state.filename || "MP4, MOV or WebM · Maximum 500 MB"}
          </div>
        ...
        {state.uploading ? (
          <div className="mt-5">
            <div className="mb-2 flex justify-between text-xs text-white/45">
              <span>Uploading to REVELATIONX1</span>
        ...
        {state.uploaded ? (
          <p className="mt-4 text-xs text-white/40">
            You can choose another file above if you want to replace this video.
          </p>
        ) : null}
      </div>
    </div>
...
function AdditionalVideoUploader({
  ...
  videoType: "ADDITIONAL_MATCH" | "FULL_MATCH";
  ...
  async function handleUpload() {
    ...
    try {
      ...
      if (!presignResponse.ok) {
        throw new Error(presign.error || "Unable to prepare video upload.");
      }

      await uploadToR2(presign.uploadUrl, file, (value) => setProgress(value));
  ...
  return (
    <div className="rounded-xl border border-dashed border-white/15 bg-black/20 p-4">
      ...
      <div className="mt-3 break-words text-xs text-white/40">
        {filename || "MP4, MOV or WebM · Maximum 500 MB"}
      </div>
      ...
      {error ? (
        <div className="mt-4 text-xs font-semibold text-red-300">{error}</div>
      ) : null}
    </div>
  );
}
...
function MoreVideosCard({
  ...
  const canAddMore = voluntaryVideos.length < MAX_VOLUNTARY_VIDEOS;
  ...
  return (
    <div className="flex h-full flex-col rounded-2xl border border-dashed border-[#c7ff2f]/50 bg-[#c7ff2f]/[0.025] p-5">
      ...
      <h2 className="mt-5 text-lg font-black">More Videos</h2>
      ...
      <p className="mt-4 text-sm leading-6 text-white/45">
        You may submit up to two additional videos if they provide useful
        evidence that is not already shown in your required footage.
      </p>

      <div className="mt-5 rounded-xl border border-white/10 bg-black/20 p-4 text-sm text-white/45">
        Additional footage submitted:{" "}
        <span className="font-bold text-white">{voluntaryVideos.length}</span> /{" "}
        {MAX_VOLUNTARY_VIDEOS}
      </div>

      {voluntaryVideos.length > 0 ? (
        <div className="mt-4 space-y-2">
          {voluntaryVideos.map((video, index) => (
            <div
              key={video.id}
      ...
      {canAddMore ? (
        <div className="mt-5">
          <AdditionalVideoUploader
            ...
            label={`Add Additional Video ${voluntaryVideos.length + 1}`}
            onUploaded={onUploaded}
          />
        </div>
      ) : (
        <div className="mt-5 rounded-xl border border-[#c7ff2f]/15 bg-[#c7ff2f]/[0.05] p-4 text-xs leading-5 text-white/50">
          You have submitted the maximum number of optional additional videos.
        </div>
      )}

      <p className="mt-5 text-xs leading-5 text-white/35">
        Further footage may still be formally requested by REVELATIONX1 if your
